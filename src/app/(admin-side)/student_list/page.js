    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await axios.delete(
                `http://localhost:5500/students/${destructuredID}`,
            );
        } catch (error) {
            console.log(error);
        }

        // Here you would perform the delete action
        // After deletion, reset selectedStudents and close the modal
        setSelectedStudents(new Set());
        setStudentNames([]); // Reset names after deletion
