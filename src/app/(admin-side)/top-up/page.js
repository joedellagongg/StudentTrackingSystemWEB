    ...
    <main className="w-full h-full p-4 rounded-2xl bg-[#ffffff]">
      <div className="flex justify-center items-center w-full h-[15%]">
        <input
          ...
          className="rounded-xl border border-[#002147] w-[50%] h-16 text-center text-xl font-bold outline-none"
        />
      </div>
      <div className="p-12 w-full h-[65%] grid grid-cols-3 gap-3">
        {top_amount.map((item) => (
          <button
            key={item.id}
      ...
      <div className="w-full h-[20%] flex justify-center items-center">
        {inputValue && (
          <button onClick={openModal} className="w-[50%] bg-[#002147] text-white rounded-xl text-3xl p-4">
            Next
          </button>
        )}
      </div>
