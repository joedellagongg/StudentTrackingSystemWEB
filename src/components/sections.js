...
export default function Section({ sections, fetchSections }) {
  ...
  return (
    <main className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 p-6 gap-3 overflow-y-scroll no-scrollbar">
      {sections.map((item) => (
        <button
          key={item.id}
          onClick={() => navigate(`/student_list?section=${item.id}`)}
          className="bg-[#002147] rounded-xl text-white text-center text-xl h-[150px] hover:bg-white border hover:text-black relative"
        >
      ...
      {modal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 lg:w-[30%] gap-y-6 rounded-xl flex flex-col justify-center items-center">
            <p>
